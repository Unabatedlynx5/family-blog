...
export const POST: APIRoute = async ({ cookies, locals }) => {
  ...
  try {
    ...
    const user = await env.DB.prepare('SELECT email, name FROM users WHERE id = ?').bind(user_id).first();
    ...
    const newAccessToken = createAccessToken({ sub: user_id, email: user.email, name: user.name }, { JWT_SECRET: jwtSecret });
